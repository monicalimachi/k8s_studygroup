...
- Step One: Attach label to the node - Label a node
```
kubectl label nodes <nde_name> <label-key>=<label-value>
kubectl label nodes node-1 size=Large
```
- Step Two: Add a nodeSelector field to your pod configuration. En el yaml file
```
apiVersion: v1
kind: Pod
metadata:
...
## NODE AFFINITY
- Ensure that pods are hosted on particular nodes. Node affinity is conceptually similar to nodeSelector -- it allows you to constrain which nodes your pod is eligible to be scheduled on, based on labels on the node.
